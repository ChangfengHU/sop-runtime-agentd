...
export async function prepareExecutionMcp(execution: Pick<ExecutionRecord, "metadata">, signal: AbortSignal): Promise<McpSession | undefined> {
  ...
  const session = new McpSession({
    ...
    authorize: async (binding: McpBinding, name: string, callSignal?: AbortSignal) => {
      ...
      try { response = await fetch(url, { redirect: "error", signal: AbortSignal.any([callSignal ?? signal, AbortSignal.timeout(15_000)]) }); }
