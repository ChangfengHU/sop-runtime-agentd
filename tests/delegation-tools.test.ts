...
test('delegation context comes from execution, never model arguments',async()=>{
 let sent:any;
 const bridge=prepareDelegation(context,new AbortController().signal,async(_url,init)=>{sent=JSON.parse(String(init?.body));return Response.json({ok:true,result:{id:'task'}})});
